...
6. A [GTPin](https://www.intel.com/content/www/us/en/developer/articles/tool/gtpin.html) Kit version 4.7.1 downloaded during setup into [`GTPin/Profilers`](./GTPin/Profilers) folder (not shipped in the repo — see [Setup](#setup) below).
...
1. Intel GPU for GTPin experiments and performance measurements:
Intel Core&trade; Ultra processor with an Intel Arc&trade; GPU
2. AMD&reg; GPU for Luthier performance measurements:
AMD EPYC&trade; 7313 16-Core Processor with an AMD
Instinct&trade; MI250 GPU
3. NVIDIA&reg; GPU for NVBit performance measurements:
Intel Xeon&trade; Platinum 8480+ with NVIDIA A100
80GB PCIe

### Intel system:

**Hardware requirements:**
1. CPU/GPU: Intel Core Ultra 5 125H processor with Intel Arc Graphics (codenamed Meteor Lake-P)

**Software requirements:**
1. A Linux&trade; OS (Ubuntu&trade; 22.04.5 LTS was used).
2. Intel GPU Driver with compute packages (v26.05.37020.3 was used).
3. Intel oneAPI&trade; C++ Essentials (v2025.2 was used).
4. OpenCL&trade; header packages.
...
2. AMD GPU Kernel Driver (v6.8.3 was used).
...
4. Intel LLVM DPC++ compiler v20.0 with NVIDIA CUDA backend support.
5. GNU C/C++ Compiler version 12.
6. Python 3 with packages `cxxheaderparser`, `pcpp`, and `yacs`.
7. CMake version 3.28.3.
...
5. Install the software dependencies based on the system you measure.
   For detailed instructions on Intel system setup, see [GTPin/README.md](./GTPin/README.md).

6. According to the system, build the instrumentation tools, assuming the previous [Setup](#setup) stage has been completed.

   For AMD, follow the detailed instructions in [`Luthier/`](./Luthier).
   For NVIDIA, follow the detailed instructions in [`nvbit_release/`](./nvbit_release).
   For detailed instructions on GTPin setup, see [GTPin/README.md](./GTPin/README.md).

### Build & Run Benchmarks

For reproduction of the GPU memory profiling described in Sections III-A (Fig. 3) and III-B, please follow the instructions in [GTPin/REPRODUCE-S3.md](./GTPin/REPRODUCE-S3.md).

The following are the instructions for reproducing Section IV - Fig. 5:
